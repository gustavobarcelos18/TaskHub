...
async function obterTokenAntiforgery(): Promise<string> {
  ...
  const resposta = await fetch("/api/autenticacao/antiforgery", {
    cache: "no-store",
    credentials: "same-origin",
  });
  if (!resposta.ok)
    throw new Error("Não foi possível preparar a requisição com segurança.");
  tokenAntiforgery = ((await resposta.json()) as TokenAntiforgery).token;
  return tokenAntiforgery;
}

export function invalidarTokenAntiforgery(): void {
  tokenAntiforgery = null;
}

export async function requisicaoComAntiforgery(
  url: string,
  init: RequestInit,
): Promise<Response> {
...
export async function obterSessao(): Promise<UsuarioAutenticado | null> {
  ...
  if (!resposta.ok) throw await criarErroHttp(resposta, "verificar a sessão");
...
export async function login(
  ...
  const resposta = await requisicaoComAntiforgery("/api/autenticacao/login", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ email, senha }),
  });
  if (!resposta.ok) throw await criarErroHttp(resposta, "entrar");
...
export async function cadastrar(
  ...
  if (!resposta.ok) throw await criarErroHttp(resposta, "criar a conta");
  invalidarTokenAntiforgery();
  return resposta.json() as Promise<UsuarioAutenticado>;
}

export async function logout(): Promise<void> {
  const resposta = await requisicaoComAntiforgery("/api/autenticacao/logout", {
    method: "POST",
  });
  if (!resposta.ok) throw await criarErroHttp(resposta, "sair");
  invalidarTokenAntiforgery();
}
