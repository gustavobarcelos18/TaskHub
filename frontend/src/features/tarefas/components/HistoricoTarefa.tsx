...
function formatarValor(
  item: HistoricoTarefaType,
  valor: string | null,
): string {
  if (item.tipo === "AlteracaoEtiquetas") {
    try {
      const nomes: unknown = valor ? JSON.parse(valor) : [];
  ...
  return (
    valor ?? (item.tipo === "AlteracaoObservacoes" ? "Sem observações" : "—")
  );
}

function ValoresAlteracao({ item }: { item: HistoricoTarefaType }) {
  if (item.tipo !== "AlteracaoObservacoes")
    return (
      <Typography variant="body2" color="text.secondary">
        {formatarValor(item, item.valorAnterior)} →{" "}
        {formatarValor(item, item.valorNovo)}
      </Typography>
    );
  ...
  return (
    <Stack spacing={0.25}>
      ...
      <Typography variant="body2" sx={sxTexto}>
        {formatarValor(item, item.valorAnterior)}
      </Typography>
      <Typography variant="caption" color="text.secondary">
        Para:
          ...
          >
            <Stack sx={{ alignItems: "center", width: 18, flexShrink: 0 }}>
              ...
              {indice < historico.length - 1 && (
                <Box
                  aria-hidden="true"
                  sx={{ width: 2, flexGrow: 1, my: 0.5, bgcolor: "divider" }}
                />
              )}
            </Stack>
            <Stack spacing={0.5} sx={{ pb: 2, minWidth: 0 }}>
              <Typography variant="caption" color="text.secondary">
                {formatarDataHora(item.criadoEm)}
              </Typography>
              <Typography variant="body1">
                {rotulosPorTipo[item.tipo]}
              </Typography>
              {item.campo && <ValoresAlteracao item={item} />}
            </Stack>
          </Stack>
        ))}
      </Stack>
