...
import { PRAZOS_TAREFA, PRIORIDADES_TAREFA, SITUACOES_TAREFA, type ConsultaTarefas, type Etiqueta, type OrdenarTarefasPor, type SituacaoTarefa, type Tarefa, type TarefasPaginadas } from "../types/tarefa";
...
type Props = { resultado: TarefasPaginadas; consulta: ConsultaTarefas; etiquetas: Etiqueta[] };
type Opcao = { valor: string; rotulo: string };

export function TabelaTarefas({ resultado, consulta, etiquetas }: Props) {
  ...
  const semFiltros = !consulta.busca && !consulta.situacao && !consulta.prioridade && !consulta.prazo && !consulta.etiquetaId;
