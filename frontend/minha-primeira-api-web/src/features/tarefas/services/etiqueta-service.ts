...
function obterUrlEtiquetas(): string {
  ...
  if (!apiUrl) throw new Error("A variável NEXT_PUBLIC_API_URL não foi configurada.");
  return `${apiUrl}/api/etiquetas`;
}
...
export async function criarEtiqueta(nome: string): Promise<Etiqueta> {
  const resposta = await fetch(obterUrlEtiquetas(), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nome }) });
  if (!resposta.ok) throw await erro(resposta, "criar a etiqueta");
  return resposta.json() as Promise<Etiqueta>;
}

export async function excluirEtiqueta(id: number): Promise<void> {
  const resposta = await fetch(`${obterUrlEtiquetas()}/${id}`, { method: "DELETE" });
