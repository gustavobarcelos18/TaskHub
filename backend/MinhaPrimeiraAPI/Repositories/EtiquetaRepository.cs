...
public class EtiquetaRepository(AppDbContext context, IUsuarioAtual usuarioAtual) : IEtiquetaRepository
{
    public Task<List<Etiqueta>> ListarAsync(CancellationToken cancellationToken = default)
    {
        return context.Etiquetas
            .AsNoTracking()
            .Where(etiqueta => etiqueta.UsuarioId == usuarioAtual.Id)
        ...
        CancellationToken cancellationToken = default)
    {
        return context.Etiquetas
            ...
            .SingleOrDefaultAsync(
                etiqueta => etiqueta.UsuarioId == usuarioAtual.Id && etiqueta.NomeNormalizado == nomeNormalizado,
                cancellationToken
            );
    }
    ...
    {
        return context.Etiquetas
            .Where(etiqueta => etiqueta.UsuarioId == usuarioAtual.Id && ids.Contains(etiqueta.Id))
            .ToListAsync(cancellationToken);
    }
    ...
    public async Task<Etiqueta?> BuscarPorIdAsync(int id, bool rastrearAlteracoes = false, CancellationToken cancellationToken = default)
    {
        ...
        return await consulta.SingleOrDefaultAsync(etiqueta => etiqueta.Id == id && etiqueta.UsuarioId == usuarioAtual.Id, cancellationToken);
    }
