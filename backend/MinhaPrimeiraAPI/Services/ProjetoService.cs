...
{
    public async Task<List<ProjetoResponse>> ListarAsync(CancellationToken cancellationToken = default)
    {
        var projetos = await repository.ListarAsync(cancellationToken);
        return projetos.Select(Mapear).ToList();
    }
    ...
    public async Task<ProjetoResponse> CriarAsync(CriarProjetoRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new ProjetoDuplicadoException();
        }
    ...
    public static string NormalizarNome(string? nome)
    {
        ...
        {
            throw new ArgumentException("O nome do projeto deve ter no máximo 100 caracteres.", nameof(nome));
        }
