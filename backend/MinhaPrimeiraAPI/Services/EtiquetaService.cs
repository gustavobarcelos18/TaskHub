...
public class EtiquetaService(IEtiquetaRepository repository, IUsuarioAtual usuarioAtual) : IEtiquetaService
{
    ...
    public static string NormalizarNome(string? nome)
    {
        ...
        if (string.IsNullOrWhiteSpace(resultado)) throw new ArgumentException("O nome da etiqueta é obrigatório.", nameof(nome));
        if (resultado.Length > 50) throw new ArgumentException("O nome da etiqueta deve ter no máximo 50 caracteres.", nameof(nome));
        return resultado;
    }
