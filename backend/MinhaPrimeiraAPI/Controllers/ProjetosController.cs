...
public class ProjetosController(IProjetoService service) : ControllerBase
{
    ...
    public async Task<ActionResult<ProjetoResponse>> Criar([FromBody] CriarProjetoRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var projeto = await service.CriarAsync(request, cancellationToken);
            return StatusCode(StatusCodes.Status201Created, projeto);
        }
        catch (ArgumentException exception)
        {
            return Problem(detail: exception.Message, title: "Projeto inválido", statusCode: StatusCodes.Status400BadRequest);
        }
        catch (ProjetoDuplicadoException exception)
        {
            return Problem(detail: exception.Message, title: "Projeto duplicado", statusCode: StatusCodes.Status409Conflict);
        }
    }
    [HttpDelete("{id:int}")]
    ...
    public async Task<IActionResult> Excluir(int id, CancellationToken cancellationToken)
    {
        if (id <= 0)
        {
            return Problem(detail: "O ID do projeto deve ser maior que zero.", title: "ID inválido", statusCode: StatusCodes.Status400BadRequest);
        }

        return await service.ExcluirAsync(id, cancellationToken)
            ? NoContent()
            : Problem(detail: $"Nenhum projeto encontrado com o ID {id}.", title: "Projeto não encontrado", statusCode: StatusCodes.Status404NotFound);
    }
}
